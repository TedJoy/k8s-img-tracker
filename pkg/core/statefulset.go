...
func WatchStatefulSet(clientset *kubernetes.Clientset) {
	factory := informers.NewSharedInformerFactoryWithOptions(clientset, time.Minute, informers.WithTweakListOptions(func(lo *metav1.ListOptions) {
		lo.LabelSelector = config.MyFileConfig.AppKey + "/statefulset=true"
	}))
...
func ReconcileStatefulSet(clientSet kubernetes.Interface, d time.Duration, wg sync.WaitGroup) {
	defer wg.Done()
	for range time.Tick(d) {
		ReconcileStatefulSetRun(clientSet)
	}
}

func ReconcileStatefulSetRun(clientSet kubernetes.Interface) {
	objs, err := clientSet.AppsV1().StatefulSets("").List(context.TODO(), metav1.ListOptions{LabelSelector: config.MyFileConfig.AppKey + "/daemonset=true"})
	if err != nil {
		panic(err.Error())
	}
	...
	for _, item := range objs.Items {
		...
		for k, v := range *itemConfig {
			...
			if currentImageHash != upToDateImageHash {
				...
				_, err = clientSet.AppsV1().StatefulSets(item.Namespace).Patch(context.TODO(), item.Name, types.JSONPatchType, patchOpBytes, metav1.PatchOptions{})
				if err != nil {
